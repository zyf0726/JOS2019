...
boot_alloc(uint32_t n)
{
    ...
    char *addr_alloc = nextfree;
    size_t npage_alloc = ROUNDUP(n, PGSIZE) / PGSIZE;
    nextfree += npage_alloc * PGSIZE;
	return addr_alloc;
}
...
page_init(void)
{
    ...
    // all the statement 'pages[...].pp_ref = 1' can be ommited,
    // because these pages will never be freed.
    size_t i;
    ...
    size_t npages_used = (boot_alloc(0) - (void *) KERNBASE) / PGSIZE;
    for (i = EXTPHYSMEM / PGSIZE; i < npages_used; ++i) /* pages already in use */
        pages[i].pp_ref = 1;
    for (i = npages_used; i < npages; ++i) { /* free pages */
        pages[i].pp_ref = 0;
        pages[i].pp_link = page_free_list;
        page_free_list = &pages[i];
    }
...
page_free(struct PageInfo *pp)
{
    ...
    if (pp->pp_ref != 0) {
        cprintf("page_free(pn=%u):\n", pp - page_free_list);
        panic("free an page in use");
    }
    if (pp->pp_link != NULL) {
        cprintf("page_free(pn=%u):\n", pp - page_free_list);
        panic("double free");
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
    ...
    pte_t *pgtab = (pte_t *) KADDR(PTE_ADDR(*va_pde));
	return &pgtab[PTX(va)];
}
...
{
	// Fill this function in
    size_t npage = ROUNDUP(size, PGSIZE) / PGSIZE;
    for (; npage > 0; --npage) {
        pte_t *va_pte = pgdir_walk(pgdir, (void *)va, 1);
        *va_pte = pa | (perm | PTE_P);
        va += PGSIZE, pa += PGSIZE;
    }
}
...
page_remove(pde_t *pgdir, void *va)
{
	// Fill this function in
    pte_t *va_pte = NULL;
    struct PageInfo *pp = page_lookup(pgdir, va, &va_pte);
    if (pp != NULL) {
        page_decref(pp);
        *va_pte = 0;
