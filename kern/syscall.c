...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	if ((errno = envid2env(envid, &e, 1)) != 0)
		return errno;
	if ((errno = user_mem_check(curenv, tf, sizeof(struct Trapframe), PTE_U)) != 0)
		return errno;

	e->env_tf = *tf;
	e->env_tf.tf_cs |= 3;
	e->env_tf.tf_eflags |= FL_IF;
	e->env_tf.tf_eflags = (e->env_tf.tf_eflags & ~FL_IOPL_MASK) | FL_IOPL_0;
	return 0;
}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	// LAB 4: Your code here.
	struct Env *srce = NULL, *dste = NULL; int errno;
	if ((errno = envid2env(srcenvid, &srce, 1)) != 0)
		return errno;
	if ((errno = envid2env(dstenvid, &dste, 1)) != 0)
		return errno;
	...
	if ((perm & PTE_W) && !(*pte & PTE_W))
		return -E_INVAL;
	if ((errno = page_insert(dste->env_pgdir, pp, dstva, perm)) != 0)
		return errno;
	return 0;
}
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	switch (syscallno) {
	case SYS_cputs:
		return sys_cputs((const char *)a1, (size_t) a2), 0;
	case SYS_cgetc:
		return sys_cgetc();
	case SYS_getenvid:
		return sys_getenvid();
